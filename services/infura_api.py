        ...
        try:
            async with session.post(infura_url, json=payload, timeout=20) as response:
                if response.status == 200:
                    ...
                    print(f"Error: Received status {response.status}. Retrying...")
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            print(f"Request error: {e}. Retrying {attempt + 1}/{retries} after delay {delay}s.")
        await asyncio.sleep(delay * (1 ** attempt) + random.uniform(0, 1))  # Exponential backoff

    print("Max retries reached. Failed to fetch batch.")
    return None
...
async def batch_get_method_ids(tx_hashes, max_retries=50):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if retries == max_retries:
            print("Max retries reached. Could not complete batch method ID request.")
            method_ids=None
    return method_ids
