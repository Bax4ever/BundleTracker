...
from bot.utils import get_user_data, get_change_arrow
from bot.messages import generate_tax_details, generate_tx_wallet_details, show_summary, generate_summary_response
import datetime
import re
import asyncio
...
async def handle_refresh(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    print(f"User Show Summary {username} with ID {user_id} at {context.user_data["users"][user_id][message_id]["refresh_count"]}")
    print(f"Message ID HF! {message_id}")
    ...
    print(f"Old Bundle: {old_bundle_percentage}, Old Sniper: {old_sniper_percentage}|{old_market_cap}")
    ...
    print(f"New Bundle Arrow: {updated_data.bundle_arrow}, New Sniper Arrow: {updated_data.sniper_arrow}, New Market Cap Arrow: {updated_data.market_cap_arrow}")
...
async def handle_tax_details(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    print(f"User Show Summary {username} with ID {user_id} at { context.user_data["users"][user_id]["request_time"]}")
    print(f"Message Id HTx:{message_id}")
    # Retrieve data for this specific message ID
